...
UStudentPerceptorOlivierStan::UStudentPerceptorOlivierStan()
{
	PrimaryComponentTick.bCanEverTick = true;
}

void UStudentPerceptorOlivierStan::BeginPlay()
{
	Super::BeginPlay();
	
	if (auto PerceptionComp = GetOwner()->GetComponentByClass<UAIPerceptionComponent>())
	{
		PerceptionComp->OnTargetPerceptionUpdated.AddDynamic(this, &UStudentPerceptorOlivierStan::OnPerceptionUpdated);
	}
}

void UStudentPerceptorOlivierStan::OnPerceptionUpdated(AActor* Actor, FAIStimulus Stimulus)
{
	if (!Actor) return;
...
void UStudentPerceptorOlivierStan::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
...
void UStudentPerceptorOlivierStan::UpdateClosestZombie(APawn* Pawn, UBlackboardComponent* BB)
{
...
AActor* UStudentPerceptorOlivierStan::PopNextVillageHouse()
{
    //Remove old houses
...
ABaseItem* UStudentPerceptorOlivierStan::FindClosestRememberedItem(FVector CurrentLocation, EItemType DesiredType)
{
...
void UStudentPerceptorOlivierStan::ForgetPickedUpItem(ABaseItem* ItemActor)
{
    if (ItemActor)
    {
